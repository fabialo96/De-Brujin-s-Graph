...
public class Kmeri4S implements PairFlatMapFunction<String, String, String>

{
	...
	for (String word : words) {
		if (word.length() >= 3) {
		 for (int i=0; i+1 < word.length()-K+1 ; i++) {
			 String kmerS = word.substring(i+1, K+i+1);
			 String kmerC = word.substring(i, K+i+1);
             list.add(new Tuple2<String,String>(kmerS, kmerC));
             ;
		 }
         }      
